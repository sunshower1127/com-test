...
exports.compactParagraph = compactParagraph;
exports.compactTable = compactTable;
exports.cleanTable = cleanTable;
exports.cleanParagraph = cleanParagraph;
...
function compactTable(tableXml, tIdx, listIdMap) {
    const out = [];
...
function extractInlineContent(xml) {
    const parts = [];
    ...
    while ((m = re.exec(xml))) {
        if (m[0].startsWith('<LINEBREAK')) {
            ...
            const t = (0, path_resolver_1.unescapeXml)(m[1]).replace(/\n/g, '\\n').replace(/\r/g, '');
            parts.push(t);
        }
    }
    return parts.join('');
...
function cleanTable(tableXml, tableIdx, baseIndent, listIdMap) {
...
function cleanTextContent(inner) {
    ...
    while ((m = re.exec(inner))) {
        if (m[0].startsWith('<LINEBREAK')) {
            ...
            const text = (0, path_resolver_1.unescapeXml)(m[1]).replace(/\n/g, '\\n').replace(/\r/g, '');
...
function cleanPicture(inner, baseIndent) {
    ...
    if (soMatch) {
        ...
        if (sizeMatch)
            out.push(indent + '    <SIZE' + filterAttrs('SIZE', sizeMatch[1]) + '/>');
        const posMatch = picMatch[2].match(/<POSITION([^/]*?)\/>/);
        if (posMatch)
            out.push(indent + '    <POSITION' + filterAttrs('POSITION', posMatch[1]) + '/>');
        out.push(indent + '  </SHAPEOBJECT>');
    }
    const imgMatch = picMatch[2].match(/<IMAGE([^/]*?)\/>/);
    if (imgMatch)
        out.push(indent + '  <IMAGE' + filterAttrs('IMAGE', imgMatch[1]) + '/>');
    out.push(indent + '</PICTURE>');
    return out.join('\n');
}
/** 화이트리스트 기반 속성 필터 */
function filterAttrs(tagName, attrStr) {
    const whitelist = types_1.ATTR_WHITELIST[tagName];
    if (!whitelist)
        return '';
    if (whitelist === '*')
        return attrStr;
