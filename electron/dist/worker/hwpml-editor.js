...
exports.initHwpmlEditor = initHwpmlEditor;
exports.setHwpHandle = setHwpHandle;
...
let _xml = '';
let _pendingChanges = [];
let _listIdMap = {}; // path → List ID 캐시
function initHwpmlEditor(bridge) {
    _bridge = bridge;
}
function setHwpHandle(handle) {
    _hwpHandle = handle;
}
/** 현재 문서에서 HWPML2X 로드 */
function getXml() {
...
function mapListIds() {
    if (!_xml)
        throw new Error('XML not loaded. Call xml.load() first.');
...
function findCellXml(xml, parts) {
    const tableIdx = parseInt(parts[0].substring(1));
    const rowIdx = parseInt(parts[1].substring(1));
    const colIdx = parseInt(parts[2].substring(1));
    const tableRe = /<TABLE[^>]*>[\s\S]*?<\/TABLE>/g;
...
function findParaXml(xml, parts) {
...
function replaceCellRaw(xml, parts, newCellXml) {
    const tableIdx = parseInt(parts[0].substring(1));
    const rowIdx = parseInt(parts[1].substring(1));
    const colIdx = parseInt(parts[2].substring(1));
    const tableRe = /<TABLE[^>]*>[\s\S]*?<\/TABLE>/g;
...
function replaceParaRaw(xml, parts, newParaXml) {
    const paraIdx = parseInt(parts[0].substring(1));
    const sectionMatch = xml.match(/<SECTION[^>]*>([\s\S]*?)<\/SECTION>/);
    if (!sectionMatch)
        throw new Error('SECTION not found');
...
function compactParagraph(pXml, pIdx) {
    const texts = [];
...
function compactTable(tableXml, tIdx) {
    const out = [];
...
function cleanTable(tableXml, tableIdx, baseIndent) {
...
function cleanParagraph(pXml, id, baseIndent) {
    const indent = '  '.repeat(baseIndent);
    const out = [];
    // P 속성
...
function cleanTextContent(inner) {
    const parts = [];
...
function cleanPicture(inner, baseIndent) {
    const indent = '  '.repeat(baseIndent);
    const out = [];
    // PICTURE 태그
    const picMatch = inner.match(/<PICTURE([^>]*)>([\s\S]*?)<\/PICTURE>/);
    if (!picMatch)
        return indent + '<!-- PICTURE not parsed -->';
...
function filterAttrs(tagName, attrStr) {
    const whitelist = ATTR_WHITELIST[tagName];
    if (!whitelist)
        return ''; // 알 수 없는 태그 → 속성 전부 제거
    if (whitelist === '*')
        return attrStr; // 전부 유지
...
function extractBodyParagraphs(xml) {
